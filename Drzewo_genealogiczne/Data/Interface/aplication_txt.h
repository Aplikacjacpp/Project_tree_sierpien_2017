...
*1.7.2   1.08.2017   New methods added	  		     								  Mateusz Marchelewicz
*1.7.2   21.08.2017  A few modifications 	  		     							  Mateusz Marchelewicz
****************************************************************************************************************/
...
//	friend char& operator>>(std::iostream& is, char &c);
	...
	// void m_import_tree();		// import drzewa (nieuzywane)
	int m_edit_tree();			// edycja drzewa
	//int m_display_tree();		// wyswietla drzewo	(nieuzywane)
	//int m_search_tree();		// metoda do wyszukiwania ludzi po danych  (nieuzywane)
	...
	// C_element m_menu_edit_relations();		// edycja relacji humana (nieuzywane)
	int m_menu_tree();					// menu w utworzonym drzewie, wyswietlania go, edycja
	// int m_export_tree(N_striing &data);	 // export drzewa do pliku (program robi to automatycznie)
